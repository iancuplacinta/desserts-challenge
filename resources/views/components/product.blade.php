...
<li>
    <article>
            ...
            <div class="bg-red flex items-center justify-center rounded-full py-3 text-white -translate-y-1/2 w-40 mx-auto gap-4 px-3">
                <form action="{{ route('cart.removeOne', $product) }}" method="POST">
                    ...
                    <button type="submit" class="rounded-full border-2 border-white p-1">
                        <svg class="size-2.5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 2"><path fill="currentColor" d="M0 .375h10v1.25H0V.375Z"/></svg>
                    </button>
                </form>
                ...
                <form action="{{ route('cart.addOne', $product) }}" method="POST">
                    ...
                    <button class="rounded-full border-2 border-white p-1">
                        <svg class="size-2.5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 10"><path fill="currentColor" d="M10 4.375H5.625V0h-1.25v4.375H0v1.25h4.375V10h1.25V5.625H10v-1.25Z"/></svg>
                    </button>
                </form>
            </div>
        @else
        <form action="{{ route('cart.addOne', $product) }}" method="POST" class="flex justify-center -translate-y-1/2">
            ...
            <button class="bg-white border border-rose-500 rounded-full px-8 py-3 font-medium flex gap-2 items-center hover:border-red hover:text-red"
                    type="submit"
            >
