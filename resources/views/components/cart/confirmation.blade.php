...
>
    <div class="flex flex-col gap-6 my-4 bg-white p-8 rounded-xl w-120 max-w-full">
        <x-icons.confirmation class="size-12 text-green shrink-0" />

        <div>
            <h2 class="font-bold text-4xl text-rose-900">Order Confirmed</h2>
            <p class="mt-2 text-rose-400">We hope you enjoy your food!</p>
        </div>


        <div class="bg-rose-50 px-4 rounded-xl">
            <ul>
        ...
        <form action="{{ route('cart.empty-cart') }}" method="POST">
            @csrf

            <button type="submit" class="text-white bg-red hover:bg-red-dark cursor-pointer transition w-full rounded-full px-6 py-4">Start New Order</button>
        </form>
    </div>
</div>
